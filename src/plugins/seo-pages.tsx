...
function SeoHead({ routePath, meta }: { routePath: string, meta: RouteConfig }) {
    const fullTitle = meta.title ? `${meta.title} | Portfolio` : 'Portfolio'
    const baseUrl = 'https://shuurei.github.io/portfolio'
    ...
    return {
        ...
        closeBundle() {
            for (const [routePath, meta] of Object.entries(routes)) {
                const head = renderToStaticMarkup(<SeoHead routePath={routePath} meta={meta} />)
